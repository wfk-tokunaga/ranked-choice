...
/*
    Ideas: make it so the sorted table shows off how many of each votes they have.
*/


//IDEA: WHEN CREATING BALLOTS, CAN ALSO MAKE INITIAL TOTAL. LESS ITERATIONS THAT WAY.

//~~~ Add a way to check if someone has totaled the threshold to sequre a spot.
// (numBallots/(numSpots + 1)) + 1 = secureThreshold; 
...
function simulateElection() {
    ...
    document.getElementById("ballotsSection").style.display = "block";
    ...
    "Simulating an election with " + numCandidates + 
    " candidates, competing for " + numSpots +
    " positions, with " + numBallots + " votes submitted."
...
function makeBallotsTable(allBallots, numCandidates, numSpots, roundNumber) {
    ...
    for (i = 0; i < allBallots.length; i++) {
        ...
        ballotNum.innerHTML = "Voter No. " + (i+1).toString();
        ...
        for (var j = 0; j < numCandidates; j++) {
            ...
            if (j < allBallots[i].length) {
                addCell.innerHTML = allBallots[i][j];
    ...
    document.getElementById("ballotsSection").appendChild(roundNumberElem);
    document.getElementById("ballotsSection").appendChild(ballotsTable);
}
...
function calcTotals(allBallots, numCandidates, numSpots) {
    ...
    for (var i = 0; i < numCandidates; i++) {
        ...
        for (var j = 0; j < numCandidates; j++) {candRow[j] = 0;}
...
function makeTotalsTable (totals, numCandidates, numSpots, roundNumber) {
    let roundNumberElem = document.createElement("p");
    roundNumberElem.innerHTML = "Round " + roundNumber + " Totals";
    ...
    let keyCell = document.createElement("th");
    infoRow.appendChild(keyCell);
    keyCell.innerHTML = "Candidate/ Totals";

    //Create the header row
    for (var i = 0; i < numCandidates; i++) {
        let ranking = document.createElement("th");
        ranking.innerHTML = ranks[i];
        if (i < numSpots) {
    ...
    document.getElementById("totalsSection").appendChild(roundNumberElem);
    document.getElementById("totalsSection").appendChild(totalsTable);
}
...
function makeQualifyingTable(sortedQualList, numCandidates, numSpots, roundNumber, totals) {
    ...
    for (var i = 0; i < numCandidates; i++) { 
        ...
        candTerm.innerHTML = sortedQualList[i][1] + ": " + sortedQualList[i][0].toString() + topVotesNumbers;
        if (i < numSpots) {
    ...
    document.getElementById("totalsSection").appendChild(qualifyingVotes);
}
...
function createSortedList(qualVotesList, numSpots) {
    let sortedArray = qualVotesList.sort(compareTotals);
    return sortedArray;
}
...
function updateVisuals(numSpots) {
    for (var i = 0; i < numSpots; i++) {
        //document.getElementById("totalsSection").children[i + 1].setAttribute("style", "background-color: green");
        document.getElementById("infoRow").children[i + 1].setAttribute("style", "background-color: green");
    }
}
-
function nextRoundVisuals() {
    let ballotsSection = document.getElementById("ballotsSection");
    //Wanna see if the round is already hidden.
    var roundIndex = 0;
    while (ballotsSection.children[3 + 2*roundIndex].style.display == "none") {
        roundIndex += 1;
    }
    let roundText = ballotsSection.children[3 + 2*roundIndex].innerHTML;
...
//helps sort qualVotesList in non-increasing order
